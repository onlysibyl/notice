...
import {
  ...
  RevokeUserRoleAPI,
  PostEditRoleAPI,
  GetDeleteRoleAPI as DeleteRoleAPI,
  GetAddRoleAPI as AddRoleAPI,
} from "../request/api";
import { Role } from "../ts/interface";
import { Group, Member } from "../ts/interface";

/***********  选择组织  ***********/
const groupList = reactive({ arr: [] as Group[]});
...
function findByGroup() {
  ...
  deleteUserSeen.value = parseInt(groupForm.id) < 10000;
}
...
function grantUser() {
  GrantUserRoleAPI({
    member: currentGrantUserObj.data,
...
function revokeUser() {
  RevokeUserRoleAPI({
    member: currentGrantUserObj.data,
    role: revokeUserCurrentRow.value,
    groupId: groupForm.id
  }).then((res) => {
    console.log(res);
  });
...
function openEditRoleDialog(row: Role) {
  editedRoleObj.role.id = row.id;
  editedRoleObj.role.name = row.name;
}

function editRole() {
    PostEditRoleAPI({
        id: editedRoleObj.role.id,
        name: editedRoleObj.role.name
    }).then((res) => {
        console.log(res);
    });
...
function addRole() {
  AddRoleAPI({
    ...
    console.log(res);
    // 新增成功后更新角色列表
    roleList.arr.push(res.data.data);
    addRoleSeen.value = false;
  });
}
</script>