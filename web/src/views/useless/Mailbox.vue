  ...
  <el-table :data="noticeList" stripe style="width: 100%">
    ...
    <el-table-column label="操作">
      <template #default="scope">
        ...
        >详情</el-button
        >
        <el-dialog v-model="noticeDetailVisible" title="通知详情" append-to-body>
          通知编号：{{ notice.id }}
          <hr />
          通知标题：{{ notice.name }}
          <hr />
...
const noticeDetailVisible = ref<boolean>(false);
...
const reply = reactive<Reply>({ content: "" });
const replySeen = ref(true);

const showNoticeDetail = (row: Notice) => {
  const notice = row;
  noticeDetailVisible.value = true;

};

const confirmNoticeDetail = () => {
